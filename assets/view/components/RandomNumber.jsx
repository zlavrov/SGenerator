...
function RandomNumber({ name }) {
    ...
    const [nbDigits, setNbDigits] = useState(1);

    const handleSend = () => {

        api.get(`/base/random-number?count=${count}&nbDigits=${nbDigits}`).then((result) => {
            setData(result.data);
        });
    };
    ...
    return (
        <Grid container spacing={2}>
            ...
            <Grid size={12}>
                <TextField type="number" fullWidth={true} onChange={event => setNbDigits(Number(event.target.value))} value={nbDigits} label="NbDigits" variant="outlined" placeholder="1" />
            </Grid>
            <Grid size={12}>
