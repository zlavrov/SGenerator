...
function Text() {
    ...
    const [maxNbChars, setMaxNbChars] = useState(1);

    const [format, setFormat] = useState('lorem');

    const handleSend = () => {

        api.get(`/lorem/text?count=${count}&maxNbChars=${maxNbChars}&format=${format}`).then((result) => {
            setData(result.data);
        });
    };
    ...
    return (
        <Grid container spacing={2}>
            ...
            <Grid size={12}>
                <TextField type="number" fullWidth={true} onChange={event => setMaxNbChars(Number(event.target.value))} value={maxNbChars} label="MaxNbChars" variant="outlined" placeholder="1" />
            </Grid>
            ...
            <Grid size={12}>
                ...
                <FormControl fullWidth={true}>
                    ...
                    <Select labelId="format-select-label" id="format-select" value={format} label="Format" onChange={(event) => setFormat(event.target.value)}>
